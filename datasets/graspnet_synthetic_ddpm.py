...
class GraspNetSynDDPM(Dataset):
    ...
    def __init__(self, data_dir, camera = "realsense", split = 'train', **kwargs):
        ...
        """
        super(GraspNetSynDDPM, self).__init__()
        ...
        if split == 'train':
            ...
            self.sceneIds = list(range(0, 100, 25))
    ...
    def __getitem__(self, index):
        ...
        data_dict = process_data(rgb, depth, depth_gt, depth_gt_mask, cam_intrinsic, camera_type = 1,  split = self.split, image_size = self.image_size, depth_min = self.depth_min, depth_max = self.depth_max, depth_norm = self.depth_norm, use_aug = self.use_aug, rgb_aug_prob = self.rgb_aug_prob, inpainting = False, with_original = self.with_original)
        depth_scale = data_dict['depth_max'] - data_dict['depth_min']
        data_dict["depth"] = (data_dict["depth"] * depth_scale.reshape(1, 1) + data_dict['depth_min'].reshape( 1, 1)).float()
        return data_dict
    def __len__(self):
